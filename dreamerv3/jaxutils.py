...
class RelaxedOnehotCategoricalDist(tfd.RelaxedOneHotCategorical):

    def __init__(self, logits=None, probs=None, temperature=1.0, dtype=jnp.float32):
        if probs:
            raise NotImplementedError()

        self._logits = logits
        super().__init__(temperature, logits, probs, dtype)
    ...
    def sample(self, sample_shape=(), seed=None):
        sample = sg(super().sample(sample_shape, seed))
        probs = self._pad(super().probs_parameter(), sample.shape)
        # jax.debug.print("probs: {}, sample: {}", probs, sample)
        return sg(sample) + (probs - sg(probs)).astype(sample.dtype)
    ...
    #     a_logits = self._logits
    #     # pylint: disable=protected-access
    #     b_logits = other._logits
    #     # sum(p ln(p / q))
    #     return jax.reduce_sum(
    #         (jax.nn.softmax(a_logits) *
    #          (jax.nn.log_softmax(a_logits) - jax.nn.log_softmax(b_logits))),
    #         axis=-1)

@tfd.kullback_leibler.RegisterKL(RelaxedOnehotCategoricalDist, RelaxedOnehotCategoricalDist)
def _kl_categorical_categorical(a, b, name=None):
  ...
  # with tf.name_scope(name or 'kl_relaxed_categorical_relaxed_categorical'):
    # # pylint: disable=protected-access
    # a_logits = a.distribution._logits_parameter_no_checks()
    # # pylint: disable=protected-access
    # b_logits = b.distribution._logits_parameter_no_checks()
    # # sum(p ln(p / q))
    # return tf.reduce_sum(
    #     (tf.math.softmax(a_logits) *
    #      (tf.math.log_softmax(a_logits) - tf.math.log_softmax(b_logits))),
...
class ScaledSymlogDist:
    # scales values by <scale> internally

    def __init__(self, mode, dims, dist='mse', agg='sum', tol=1e-8, scale=1000.0):
        self._mode = mode
        self._dims = tuple([-x for x in range(1, dims + 1)])
        self._dist = dist
        self._agg = agg
        ...
        print(f"Using ScaledSymlogDist with scale {self._scale}")
        self.batch_shape = mode.shape[:len(mode.shape) - dims]
        self.event_shape = mode.shape[len(mode.shape) - dims:]

    def mode(self):
        return symexp(self._mode) / self._scale
    ...
    def log_prob(self, value):
        assert self._mode.shape == value.shape, (self._mode.shape, value.shape)
        if self._dist == 'mse':
            distance = (self._mode - symlog(value * self._scale)) ** 2
            distance = jnp.where(distance < self._tol, 0, distance)
        elif self._dist == 'abs':
            distance = jnp.abs(self._mode - symlog(value * self._scale))
            distance = jnp.where(distance < self._tol, 0, distance)
        else:
            raise NotImplementedError(self._dist)
        if self._agg == 'mean':
...
class Optimizer(nj.Module):
    ...
    def __call__(self, modules, lossfn, *args, has_aux=False, skip_keys_pattern=None, **kwargs):
        ...
        loss, params, grads, aux = nj.grad(
            wrapped, modules, has_aux=True, skip_keys_pattern=skip_keys_pattern)(*args, **kwargs)
