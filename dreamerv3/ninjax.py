...
def grad(fun, keys, has_aux=False, skip_keys_pattern=None):
    ...
    def wrapper(*args, **kwargs):
        ...
        if skip_keys_pattern:
            filtered_strs = []
            for s in strs:
                if re.match(skip_keys_pattern, s):
                    print(f"not optimizing key: {s}")
                else:
                    filtered_strs.append(s)
            strs = filtered_strs
            print(f"optimizing keys: {strs}")
