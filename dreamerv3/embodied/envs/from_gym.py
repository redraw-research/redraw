...
class FromGym(embodied.Env):
    ...
    def __init__(self, env, obs_key='image', act_key='action', **kwargs):
        if isinstance(env, str):
            ...
            self._env = env
        self._obs_dict = hasattr(self._env.observation_space, 'spaces') or isinstance(self._env.observation_space, dict)
    ...
    def obs_space(self):
        if self._obs_dict:
            spaces = self._flatten(self._env.observation_space.spaces) if not isinstance(self._env.observation_space, dict) else self._env.observation_space
        else:
