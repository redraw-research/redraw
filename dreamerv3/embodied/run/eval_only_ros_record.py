...
def eval_only_ros_record(agent, env, logger, args):
    ...
    num_eval_episodes = max(len(env), args.eval_eps)
    ...
    rosbag_topics_list = [
         "/tf",
         "/tf_static",
         "/duckiebot5/camera_node/image_raw", # this will increase the size a lot
    ...
    def per_episode(ep):
        ...
        if len(scores_from_current_evaluation) >= num_eval_episodes:
            print(scores_from_current_evaluation)
            logger.add({
        ...
        user_input = input(f"Step {step}: n to drop episode, anything else to add episode")
