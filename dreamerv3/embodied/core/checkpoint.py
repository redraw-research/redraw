...
from dreamerv3.embodied.core import basics
...
class Checkpoint:
    ...
    def exists(self, filename=None):
        ...
        self._log and not exists and print(f'Did not find any checkpoint {filename}.')
        return exists
    ...
    def load(self, filename=None, keys=None, skip_key_prefixes: Optional[Iterable] = None):
        skip_key_prefixes = skip_key_prefixes or []
        assert self._filename or filename
        filename = path.Path(filename or self._filename)
        ...
        for key in keys:
            skip = False
            for skip_key_prefix in [*skip_key_prefixes, '_']:
