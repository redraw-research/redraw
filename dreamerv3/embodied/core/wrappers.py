...
class FrameStackOnlyImage(base.Wrapper):

    def __init__(self, env, stack_size=2, return_as_framestacked_image=False):
        super().__init__(env)
        self._stack_size = int(stack_size)
        self._stack = deque(maxlen=self._stack_size)
        self._key = "image"
        if return_as_framestacked_image:
            self._key = "framestacked_image"
    @functools.cached_property
    def obs_space(self):
        spaces = self.env.obs_space
        assert "image" in spaces
        num_channels = spaces['image'].shape[2] * self._stack_size
        shape = spaces['image'].shape[:2] + (num_channels,)
        spaces[self._key] = spacelib.Space(np.uint8, shape)
        return spaces

    def step(self, action):
        obs = self.env.step(action)
        if obs['is_first']:
            self._stack.append(obs["image"])
        obs[self._key] = self._stack_image(obs["image"])
        return obs

    def _stack_image(self, image):
        self._stack.append(image)
        return np.concatenate(self._stack, axis=2)