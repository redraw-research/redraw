...
from dreamerv3.embodied.core import path
...
class Logger:
    ...
    def __init__(self, step, outputs, multiplier=1):
        ...
        print(f"Logging step multiplier is {self.multiplier}")
...
class TerminalOutput:
    ...
    def __call__(self, summaries):
        ...
        if self._console:
            if self._name:
                self._console.rule(f'[green bold](Step {step}) {self._name}')
            else:
                self._console.rule(f'[green bold]Step {step}')

            # Original logging print-to-screen:
            # self._console.print(' [blue]/[/blue] '.join(
            #     f'{k} {v}' for k, v in formatted.items()))

            # New logging prints items as rows in a vertical list:
            report = ""
            last_top_level_key = None
            for k, v in formatted.items():
                top_level_key = k.split("/")[0]
                if top_level_key != last_top_level_key:
                    last_top_level_key = top_level_key
                    report = report + "\n"
                report = report + f"{k} {v}\n"
